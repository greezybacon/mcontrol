...
class parser(object):
    ...
    def parseLine(self, textline, pattern, resultSoFar = [], skipWS = True, skipComments = None):
        ...
        def R(result, text):
            ...
            if name and result:
                ...
                name.line = self.lineNo(source_pos)
            elif name:
        ...
        if callable(pattern):
            ...
            if pattern.__name__[0] != "_":
                ...
                source_pos = self.textlen - len(textline) + 1
    ...
    def lineNo(self, parsed):
        if not(self.lines): return u""
        if not parsed: return u""
